...
class DivisionDecimaux(TemplateExercice):
    def __init__(self):
    ...
    def generate(self):
        ...
        for exercice in range(self.nbExercice):
            ...
            for question in range(self.nbQuestion):
                ...
                tempsolution.append("\\shadowbox{\\opdiv[displayshiftintermediary=all,displayintermediary=nonzero,hfactor=decimal]%s}{%s}}" % (first*10**exp1, second*10**exp2))
            self.listeEnonce.append(tempenonce)
            self.listeSolution.append(tempsolution)


if __name__ == "__main__":
    app = DivisionDecimaux()
    app.display()